...
要在工具执行前进行精准拦截，最糟糕的做法是跑去修改 bash.go 或者 edit_file.go 的内部代码，写一堆 if command == "rm"。这不仅会污染业务逻辑，还会破坏我们在第 5 讲中建立的“高内聚低耦合”的 Registry 架构。
...
打开 internal/feishu/bot.go，在事件调度器中增加对 approve 和 reject 命令的拦截。同时，相较于第 9 讲的实现，此次 feishu/bot.go 也要针对第 11 讲新增的 session 做一些改造：
```
